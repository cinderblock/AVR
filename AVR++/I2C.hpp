...
volatile inline Status getStatus() { return (Status)(SR->Status << 3); }
...
public:
  inline Control(bool useInterrupt = true, bool ack = false) : crValueBase(0b00000100 | (useInterrupt << TWIE)) {
    CR->byte = crValueBase | (ack << TWEA);
  }
  inline void clearFlag() { CR->byte = crValueBase | (1 << TWINT); }
  inline void clearFlagEnableAck() { CR->byte = crValueBase | (1 << TWINT) | (1 << TWEA); }
  const inline void enableAck() { CR->byte = crValueBase | (1 << TWEA); }
};

inline static void sendByte(u1 const b) { *DR = b; }

inline static u1 getByte() { return *DR; }

inline static void sendSTART() { CR->byte = 1 << TWINT | 1 << TWEN | 1 << TWSTA; }

inline static void sendSTOP() { CR->byte = 1 << TWINT | 1 << TWEN | 1 << TWSTO; }

inline static void setupACK() { CR->byte = 1 << TWINT | 1 << TWEN | 1 << TWEA; }

inline static void setupNACK() { CR->byte = 1 << TWINT | 1 << TWEN; }
...
inline static void setBitRateRegister(u1 twbr) { *BR = twbr; }

inline static bool isWriteAddress(u1 const addr) { return (addr & 1) == 0; }

inline static bool isReadAddress(u1 const addr) { return (addr & 1) == 1; }

inline static u1 makeWriteAddress(u1 const addr) { return addr & ~1; }

inline static u1 makeReadAddress(u1 const addr) { return addr | 1; }
}; // namespace I2C
}; // namespace AVR
